...
local function toString(...)
    ...
    for i, v in ipairs(data) do
        ...
        elseif dataType == "Axes" then
            results[i] = string.format("Axes.new(%s)", genEnumList(v, Enum.Axis, ","))
        ...
        elseif dataType == "Color3" then
            results[i] = string.format("Color3.fromRGB(%u,%u,%u)", (v.R*255)+0.5, (v.G*255)+0.5, (v.B*255)+0.5)
        elseif dataType == "ColorSequence" then
            ...
            if #keypoints == 2 and keypoints[1].Value == keypoints[2].Value then
                ...
                results[i] = string.format("ColorSequence.new(Color3.fromRGB(%u,%u,%u))", (c.R*255)+0.5, (c.G*255)+0.5, (c.B*255)+0.5)
            elseif #keypoints == 2 then
                local c1 = keypoints[1].Value
                local c2 = keypoints[2].Value
                results[i] = string.format("ColorSequence.new(Color3.fromRGB(%u,%u,%u),Color3.fromRGB(%u,%u,%u))", (c1.R*255)+0.5, (c1.G*255)+0.5, (c1.B*255)+0.5, (c2.R*255)+0.5, (c2.G*255)+0.5, (c2.B*255)+0.5)
            else
                results[i] = string.format("ColorSequence.new({%s})", toString(unpack(keypoints)))
            end
        elseif dataType == "ColorSequenceKeypoint" then
            local c = v.Value
            results[i] = string.format("ColorSequenceKeypoint.new(%s,Color3.fromRGB(%u,%u,%u))", tostring(round(v.Time, 7)), (c.R*255)+0.5, (c.G*255)+0.5, (c.B*255)+0.5)
        elseif dataType == "EnumItem" then
            results[i] = tostring(v.Value)
        elseif dataType == "Faces" then
            results[i] = string.format("Faces.new(%s)", genEnumList(v, Enum.NormalId), ",")
        elseif dataType == "NumberRange" then
            results[i] = string.format("NumberRange.new(%s,%s)", tostring(round(v.Min, 7)), tostring(round(v.Max, 7)))
        ...
        elseif dataType == "NumberSequenceKeypoint" then
            if v.Envelope == 0 then
                results[i] = string.format("NumberSequenceKeypoint.new(%s,%s)", tostring(round(v.Time, 7)), tostring(round(v.Value, 7)))
            else
                results[i] = string.format("NumberSequenceKeypoint.new(%s,%s,%s)", tostring(round(v.Time, 7)), tostring(round(v.Value, 7)), tostring(round(v.Envelope, 7)))
            end
        elseif dataType == "PhysicalProperties" then
            ...
            if fWeight == 1 and eWeight == 1 then
                results[i] = string.format("PhysicalProperties.new(%s,%s,%s)", tostring(round(v.Density, 7)), tostring(round(v.Friction, 7)), tostring(round(v.Elasticity, 7)))
            else
                results[i] = string.format("PhysicalProperties.new(%s,%s,%s,%s,%s)", tostring(round(v.Density, 7)), tostring(round(v.Friction, 7)), tostring(round(v.Elasticity, 7)), tostring(round(fWeight, 7)), tostring(round(eWeight, 7)))
            end
        elseif dataType == "Ray" then
            local origin, dir = v.Origin, v.Direction
            results[i] = string.format("Ray.new(Vector3.new(%s,%s,%s),Vector3.new(%s,%s,%s))", tostring(origin.X), tostring(origin.Y), tostring(origin.Z), tostring(dir.X), tostring(dir.Y), tostring(dir.Z))
        elseif dataType == "Rect" then
            results[i] = string.format("Rect.new(%s,%s,%s,%s)", tostring(v.Min.X), tostring(v.Min.Y), tostring(v.Max.X), tostring(v.Max.Y))
        elseif dataType == "UDim" then
            results[i] = string.format("UDim.new(%s,%s)", tostring(round(v.Scale, 7)), tostring(round(v.Offset, 7)))
        elseif dataType == "UDim2" then
            results[i] = string.format("UDim2.new(%s,%s,%s,%s)", tostring(round(v.X.Scale, 7)), tostring(round(v.X.Offset, 7)), tostring(round(v.Y.Scale, 7)), tostring(round(v.Y.Offset, 7)))
        elseif dataType == "Vector2" then
            results[i] = string.format("Vector2.new(%s,%s)", tostring(v.X), tostring(v.Y))
        elseif dataType == "Vector3" then
            results[i] = string.format("Vector3.new(%s,%s,%s)", tostring(v.X), tostring(v.Y), tostring(v.Z))
        else
...
local function toStringVerbose(...)
    ...
    for i, v in ipairs(data) do
        ...
        elseif dataType == "ColorSequenceKeypoint" then
            ...
            results[i] = string.format("ColorSequenceKeypoint.new(%s, Color3.fromRGB(%u, %u, %u))", tostring(round(v.Time, 7)), (c.R*255)+0.5, (c.G*255)+0.5, (c.B*255)+0.5)
        elseif dataType == "EnumItem" then
            results[i] = string.format("Enum.%s.%s", tostring(v.EnumType), v.Name) --verbose
        elseif dataType == "Faces" then
            results[i] = string.format("Faces.new(%s)", genEnumList(v, Enum.NormalId), ",")
        elseif dataType == "NumberRange" then
            results[i] = string.format("NumberRange.new(%s, %s)", tostring(round(v.Min, 7)), tostring(round(v.Max, 7)))
