...
local serializePluginGui = plugin:CreateDockWidgetPluginGui(
	"dekkonot-instance-serializer-main",
	DockWidgetPluginGuiInfo.new(
		Enum.InitialDockState.Float, --initDockState
		true, --initEnabled
		false, --overrideEnabledRestore
		250,
		200, --floatXSize, floatYSize
		0,
	...
	elseif #currentSelection > 1 then
		for _, v in ipairs(currentSelection) do
			...
			if didSerialize then
				output.Name = "Serialized_"
					.. string.gsub(v.Name:gsub("[^%w_]+", ""), "^%d+", "")
...
firstLoadConnection = serializePluginGui
...
local toggleButton = toolbar:CreateButton(
	"dekkonot-instance-serializer-toggle",
	"Toggle the serializer widget",
	"rbxassetid://2794885159",
	"Toggle Widget"
)
local action = plugin:CreatePluginAction(
	"dekkonot-instance-serializer-run",
	"Serialize Selected Instances",
	"Run the serializer with the current settings",
	"rbxassetid://2795131004",
	true
)
